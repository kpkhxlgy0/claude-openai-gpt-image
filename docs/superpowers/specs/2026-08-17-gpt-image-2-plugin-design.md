...
- Required: yes
- Sensitive: yes
- Purpose: authentication for the local image MCP server
...
- Returns `CONFIG_MISSING` before any paid request when the API key is absent.
...
If exactly one allowed root exists, tools select it automatically. If multiple roots exist:

- An optional `workspace_root` argument may select one of the roots already published by the client.
- The argument does not grant a new root.
...
- `workspace_root`: optional selector when the client has published multiple roots
...
4. Reject traversal, path-prefix collisions, NUL bytes, Windows drive-relative paths, UNC paths, device paths, alternate data streams, reserved device names, and mixed-separator escapes.
5. Reject symlink or Junction escapes in existing ancestors.
6. Re-check canonical containment after creating output directories.
7. Never follow an output leaf symlink.
...
Input images are opened through stable file handles. Edit inputs are copied to immutable, bounded snapshots under `${CLAUDE_PLUGIN_DATA}` or an OS-secure temporary directory before the network request. The snapshots are deleted after the request finishes. This prevents an input file from being swapped between validation and upload.
...
- Allows at most one paid image request at a time per MCP process.
...
- Relay only paths and metadata actually returned by the MCP server.
- Never claim success after an error.
- Never invent previews, costs, dimensions, formats, request IDs, or saved files.
- Present inline previews when supplied and clickable paths otherwise.
- Keep security warnings and provider mismatch warnings visible.

### `commands/setup.md`

A user-facing setup command invokes only `get_status`. It verifies tool availability, local configuration presence, Base URL validity, workspace roots, and the default output directory. It must not call OpenAI or perform a paid generation.
...
Build output is deterministic apart from expected source-map or banner metadata, which will be disabled or normalized for release verification.
...
- Runtime paths use `${CLAUDE_PLUGIN_ROOT}` and `${CLAUDE_PROJECT_DIR}`.
...
- Rejection of whitespace, control characters, credentials, query strings, fragments, and unsupported schemes.
...
- Stable input handles and immutable snapshots.
...
- Claude Code CLI installation as a secondary path.
